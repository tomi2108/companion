...
export class DeployYaml extends YamlFile<DeployYamlContent> {
  ...
  setConfigMap(name: string) {
    ...
    const values = Object.values(configmaps);
    if (values.includes(name)) return;
    const i = values.length;
  ...
  getSecrets() {
    return Object.values(this.read()["helm-chart-master"].secrets ?? {});
  }

  getConfigmaps() {
    return Object.values(this.read()["helm-chart-master"].configmaps ?? {});
  }
