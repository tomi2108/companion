...
export class RepoInstall extends WorkflowStep<Reads, Writes, Options> {
  ...
  async run(ctx: ExecutionContext, { app_repo, version, source_branch, dependency }: Reads) {
    ...
    await app_repo.stash(async () => {
      ...
      const new_branch_name = `bump/${dependency}-${version}`;
      ...
      const app_repo_remote = new RepoWithGitProvider(app_repo, ctx.gitProvider);
      if (merge) await app_repo_remote.createAndMergeMr(source_branch);
      else await app_repo_remote.createMr(source_branch);
      await app_repo.switchBranchIfExists(source_branch);
