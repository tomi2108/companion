...
export class GetAppLatestVersion extends WorkflowStep<Reads, Writes> {
  ...
  async run(_: ExecutionContext, { app_repo }: Reads) {
    ...
    // TODO[https://gitlab-ee.agil.movistar.com.ar/movar_app/tools/dux/-/issues/70]: this is only right for backend deployments
    // for frontend deployments we should look for -beta, -rc for different namespaces
    // find a good way to represent this in the config, this should be used in dux app status as well
