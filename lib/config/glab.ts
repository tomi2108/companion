...
export class GitlabConfig implements IntegrationConfig, Schema {
  ...
  token: Schema["token"] = "";
  server = process.env.CLAIR_GLAB_SERVER ?? "";
  ms_template_id = Number(process.env.CLAIR_GLAB_MS_TEMPLATE_ID) ?? -1;
  mf_template_id = Number(process.env.CLAIR_GLAB_MF_TEMPLATE_ID) ?? -1;
